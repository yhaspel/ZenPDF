...
3. **Baseline must be green before you change anything:**
   ...
   ```
   Expected: backend **232 passed**, frontend **40 passed**, e2e **7 passed**. If the ONLY failure is the known `e2e/phase-2.spec.ts` flake (next item), fix it and re-run; any other red = 🟡 Blocker in PROGRESS.md, STOP.
