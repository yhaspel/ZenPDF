...
> **Historical note, not an instruction (softened 2026-08-22).** This block was written on 2026-08-01 to unblock a prompt that was superseded the same day and never run; it is kept because it records the shape Phase 2B left behind, and it should be read as description rather than as a step to take. What it says is still true of the code: ownership flows through `apps/core/principals.py` (01-architecture **§21**, normative) and a grep test fails the build on `request.user` / `job.user` / `owner=` / `context["request"].user` outside that module. The conventions it lists are the ones phases 3–7 went on to follow: scope querysets with `owned_by(qs, principal)`, create rows with `owner_kwargs(...)`, resolve worker ownership with `principal_of(job)`, put new limits in `settings.TIERS` behind `core.limits.for_principal()`, and leave `IsPrincipal` as the default permission unless a feature is genuinely account-only (§21.3, with a written reason). Each of those phases also shipped its public tool page by appending to `frontend/src/app/core/tool-pages.ts` — routes, prerendering and `sitemap.xml` are generated from that table.
