...
1. **A page is not drawn until its pixels say so.** No test may conclude that a document rendered from a 200, from the presence of a viewer element, or from a `pdfLoaded`-class event. The workspace exposes `data-test=viewer-drew` (pdf.js `pageRendered`) and `data-test=overlay-drew` (the page raster decoded with a non-zero natural size), both keyed to the URL that drew so a save or a revert withdraws the claim; `e2e/tests/drew.ts` waits for the marker and then reads the canvas back, asserting more than three distinct colours across rows sampled at a 97 px stride. Production rendered nothing at `/app/doc/:id` for ten days while every existing check stayed green.
