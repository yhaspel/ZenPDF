...
- **Installable app / service worker** *(added 2026-09-26)*: Angular's service worker, wrapped by `public/zen-sw.js`, caches **only the static build**. It prefetches the shell (`index.csr.html`) and hashed chunks, and lazily caches fonts, icons and pdf.js assets. `/api/**` and every other origin are taken out of the worker's hands entirely, so per-user data is never in a cache and uploads and ranged reads run as if there were no worker. Returning visitors are served the CSR shell by the worker; crawlers, which run no worker, still get the prerendered pages. A new deploy is applied at the next navigation outside the workspace, request builder and ceremony, never mid-upload. Operational detail: `AGENTS.md` *Installable app (PWA)*.
...
Django email → SMTP env (Mailpit in dev). HTML+text templates: sign invite, reminder, declined, completed (+links), email-verification. Beat: `sign_reminders` hourly *(amended 2026-08-02, P8: the cadence a user chooses is measured in days, so the task only has to notice that a day has passed — hourly does that with no clock-time assumption about anybody's timezone, and a stuck run is visible sooner)* (pending recipients, every `reminder_every_days`, stop at expiry) · `sign_expirations` hourly · `trash_purge` daily (trashed >30 d) · `exports_purge` **hourly** (>24 h) *(corrected 2026-08-02: §15 said daily; the schedule has run it hourly since phase 6 — the doc was wrong, not the code)* · **`guest_purge` hourly** (expired GuestSessions → hard-delete their documents, versions, thumbs and blobs; §21.4) · **`usage_recompute` daily 02:00** *(corrected 2026-08-02 to "never implemented"; **implemented 2026-08-23**. It recomputes `storage_bytes_used` for every `User` and every **live** `GuestSession` from what is actually charged — every `DocumentVersion.size_bytes` of documents they own **including trashed ones** (trash costs until `_purge` credits it back, which `trash_purge` does not do for thirty days), plus the blobs under `uploads/{u|g}/{id}/` (which have no rows at all, so the prefix is the only source of truth), plus exports whose blob still **exists** — presence read from storage and the amount from the row, which is exactly `jobs_purge`'s rule and for its reason: `exports_purge` refunds and then strips the key in two un-transacted steps. Absolute `UPDATE`s, never `F()` — a relative update would carry the drift it exists to remove. It **skips any principal with a queued or running job** (the counter is mid-flight) and any principal whose counter moved while it was counting, and re-checks both inside the lock; expired guest sessions are skipped because `guest_purge` deletes them within the hour. Drift ≥ 1 KB is logged with before/after. Companion command `manage.py recompute_usage [--principal <uuid>] [--dry-run|--apply]`, where **dry run is the default** — the beat task heals unattended, but a wrong recompute run by hand is a wrong quota for every user at once)* · **`job_params_purge` daily** (blank the *inputs* of terminal jobs older than 30 d — patterns, replacement text, page lists; the row, and so the user's history, stays) · **`jobs_purge` daily** (delete terminal job rows older than `JOB_RETENTION_DAYS`, after their `exports/{job_id}/` prefix and with the bytes refunded) · **`account_assets_purge` daily 03:00** *(added 2026-08-23: `uploads/u/{id}/` blobs older than `ASSET_RETENTION_DAYS` (7), with the bytes refunded. `purge_principal_assets` ran only for guest purge and account deletion, so a guest's stamps died with the session inside the hour and an **account's were charged for ever** with no screen that could free them. **Saved signatures are exempt by key**: §13 deliberately put `SavedSignature.storage_key` in this same prefix so signatures would inherit its metering, key derivation and guest purge, so age alone would have deleted a stored image of somebody's signature and orphaned the row. Guest prefixes are left to `guest_purge`, which is sooner and takes the whole prefix — two sweeps on the same blobs would refund the same bytes twice)*. *(`version_retention_gc` removed 2026-08-02 — see §14.)*
...
Backend: pytest + pytest-django, `CELERY_TASK_ALWAYS_EAGER=True` for API tests; engine functions get **golden tests** against a fixture corpus (`tests/fixtures/pdfs/`: text.pdf, scanned.pdf (image-only), form.pdf (AcroForm), encrypted.pdf, rotated-90.pdf, unicode.pdf, corrupt.pdf, large-generated.pdf 500 pp) asserting page counts, extracted text, form values, encryption state — not byte-equality (PDF output nondeterminism). Coverage gate: apps 85%, pdf_engine 90%. Frontend: unit tests for every facade + core service (HttpTestingController); `ng test` with the v22 default runner ⚠. E2E: Playwright against the full compose stack (`infra/test.sh --e2e`), one happy-path spec per phase (see phase docs). CI note: any CI must run `ruff`, `mypy` (loose), **`eslint` (type-aware)**, backend tests, frontend tests.

⚠ **`eslint` here means the type-checked config** *(2026-08-24, `chore/type-aware-eslint`)*. `frontend/eslint.config.js` extends `typescript-eslint`'s `recommendedTypeChecked` + `stylisticTypeChecked` with `parserOptions.projectService`, so the lint builds a TypeScript program and a CI that runs it needs the tsconfigs and `node_modules` present, not just the sources. It costs **+5.5 s** in this project's own container (11.5 s → 17 s, measured twice each) — the reason it is in the gate rather than deferred to a nightly. `strictTypeChecked` was measured against the same tree and **not** adopted; the four rules that account for most of the difference are named in `eslint.config.js` with what each fires on here.
...
ASSET_RETENTION_DAYS=7                   # uploads/{u|g}/… ephemera (§13); saved signatures exempt
