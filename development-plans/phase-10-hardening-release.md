...
- **E2E regression suite:** the per-phase Playwright specs consolidated into a tagged suite (`@smoke` ~10 min: auth, upload, one op, self-sign, ceremony happy path; `@full` nightly: everything incl. time-warped reminders); flake budget zero-tolerance policy (quarantine tag + fix-within-phase rule). *(2026-08-22, as built: `@smoke` exists and runs in ~30 s; **there is no `@full` tag** — the nightly run is the whole suite untagged, with `BROWSERS=all`. And `@quarantine` is a tag nothing acts on: `playwright.config.ts` has no `grepInvert` and `infra/test.sh` runs the suite bare, so a quarantined test still runs and still fails the gate. Both are `docs/reviews/handoffs/handoff-to-cli-e2e-gate-hardening.md`'s to fix.)*
...
- [ ] `@full` suite green 3 consecutive nightly runs on the prod-shaped stack. *(2026-08-22: **no `@full` tag exists.** `@smoke` is real and is the deploy gate; "`@full`" is how this document names **the untagged whole suite** — `./infra/test.sh --e2e` with no grep, 63 tests today. Read the criterion as "the whole suite, three consecutive nightly runs".)*
- [ ] ~~Clean-VM prod compose deploy documented + performed~~; restore drill performed. *(**Re-scoped 2026-08-22.** The deploy half is retired: production is **Railway**, deployed 2026-08-08 and auto-deploying from `main` since 08-10 (`docs/ops/railway-deploy-report-2026-08-08.md`). A compose deploy on a clean VM was never performed and is no longer the target — proving out a stack nobody operates would evidence nothing. **The restore drill stands and is the live half of this criterion**, on the Railway procedure: volume snapshot plus managed-Postgres restore, keeping the audit-chain re-verification step. It needs a Railway token, so it is an owner item.)*
