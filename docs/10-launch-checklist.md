...
> **Status 2026-08-25** *(`docs/launch-gate-evidence`; full write-up in `docs/reviews/evidence/launch-gate-2026-08-25.md`, which supersedes the 2026-08-21 note below on all four points).* Three of the five boxes above now have the evidence they asked for. Nobody has ticked them for you, because two of the three end in a judgement and the judgement is yours.
>
> - **Whole suite, three consecutive runs — evidence exists, two words of the box do not match it.** Three `./infra/test.sh --pg --e2e` runs, each from a fresh `reset.sh --yes && up.sh`, **all exit 0**: 8m23s / 8m37s / 8m27s, identical numbers every time (pytest **1164 passed, 6 skipped**; the `--pg` leg **6 passed**, so the skips were exercised and not lost; frontend unit **66 files / 576 tests**; Playwright **86 passed, 1 skipped**). **No flake to record.** **Two gaps, named rather than rounded away.** *"Three consecutive runs"*: these are back to back inside one hour on one machine — not three nights, which is what a nightly is for (a cold machine, a new day's containers, a fresh clock). *"On the prod-shaped stack"*: these ran against the compose stack, which is `manage.py runserver`, not gunicorn behind nginx; the deployed shape is covered by `@smoke` against production instead, which is a smaller thing. Tick it only if you read the box as "three consecutive green runs of the whole suite"; as literally written it is not met.
> - **`@smoke` against the deployed host — 5 passed, 2 environmental, 0 real.** `phase-8:135` polls Mailpit on `localhost:8025`, which production has no equivalent of while SMTP is off. `smoke-viewer` timed out **inside the suite**, which runs from one IP against production's real rate limiter — its page snapshot shows the *upload* stalled on `/annotate-pdf`, not a blank viewer, and run standalone against production it passes in **3.8 s** printing `canvas 1056×1494, 64 distinct colours`. Both re-run green against the local stack, so the specs are sound. **`phase-8:91`, environmental on 2026-08-21, now passes in-suite.** Whether "5 of 7 with two environmental" is green is the judgement, and it is still yours.
> - **Lighthouse ≥ 90 on the deployed build — the landing half is done; the dashboard half needs a decision about the criterion.** Landing on production: **desktop 100/100/100/100, mobile 94/100/100/100**; `/merge-pdf` and a guide **100 across all four**. The dashboard cannot be run against the host without logging a real account into the live service, so it was measured on a local **production build** behind this repo's own `nginx.conf`: **96 perf / 96 a11y / 100 best-practices / 69 SEO**. **The SEO number is the product being correct** — the only failing audit is `is-crawlable`, and `/app/dashboard` is deliberately noindex (§7), so this box *as written* cannot be ticked without making the product worse. The a11y 96 is a real gap: `target-size`, measured at **17 × 17 px** (card select checkbox) and **28 × 28 px** (card ⋯ menu) against the design contract's **≥ 44 px**; it has its own queue row.
> - **The p95** is not a box here, but it moved: all six §10.2 endpoints pass the 150 ms budget at **6–12 ms** locally over 3079 requests with 0 failures. The production figure still needs `PERF_EMAIL` and a decision to point a load generator at the live service — owner item 8.
> - **GATE rows:** unchanged by this work. H1 (production signing certificate) closed 2026-08-23; the Privacy/Terms and e-sign disclosure reviews remain open. `v1.0.0` is not tagged, which is correct.
>
