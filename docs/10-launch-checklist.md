...
>
> - **Whole suite, three consecutive runs — evidence complete.** Three `./infra/test.sh --pg --e2e` runs, each from a fresh `reset.sh --yes && up.sh`, **all exit 0**: 8m23s / 8m37s / 8m27s, identical numbers every time (pytest **1164 passed, 6 skipped**; the `--pg` leg **6 passed**, so the skips were exercised and not lost; frontend unit **66 files / 576 tests**; Playwright **86 passed, 1 skipped**). **No flake to record.** The judgement: "nightly" here means the cadence in `docs/ops/release.md`, and these are three consecutive runs back to back on one machine. If you read the word as *three different nights*, this is not that.
