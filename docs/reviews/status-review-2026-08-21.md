...
|---|---|---|---|---|
| 19 | Phase-12 section and session log (`:68`, `:1139`, `:1156`) | "`data-test` diff **0 removed, 27 added**" | A plain attribute diff of `frontend/src` between `f8743ac` and `f34800f` shows **30** added (§0 lists them) and 0 removed — the record undercounts by three | Cosmetic; say 30 or say which three were excluded. *(Corrected 2026-08-22 while applying this item: the guess that followed — `area-select`, `builder-field-select`, `placement-select` — was wrong; all three of those are in the 27. The three actually missed are `shortcuts-help`, `shortcut-row` and `shortcuts-close`, because the handoff's command globs `frontend/src/**/*.html` and `ShortcutsHelp` keeps its template inline in a `.ts`. §0's list above was right; only this parenthetical was not.)* |
