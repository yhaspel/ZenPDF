...
>
> **Three of its instructions were wrong and were not followed as written.** (1) §4 says "saved signatures live elsewhere" — they do not; §13 puts `SavedSignature.storage_key` in the very prefix being swept, so the sweeper exempts them by key or it would delete a stored image of somebody's signature. (2) §6's local verification (`curl` :4200 with the api stopped) proves nothing: :4200 in the compose stack is the Angular dev server, not nginx — the 503 was proven against the real config with a dead-but-resolvable upstream instead. (3) §7's test could not open the file it asserts about, because the api container mounts `backend/` and nothing else; `infra/railway/` is now mounted read-only rather than the number being copied into `backend/`, which is the defect being fixed. Kept as written below, for the reasoning.
