**Executed 2026-08-25 — the launch-gate evidence a machine can produce is produced, and two of the three criteria it targeted still cannot be ticked, for reasons worth reading. **Lighthouse**: landing on the deployed build **100/100/100/100** desktop and **94/100/100/100** mobile, a tool page and a guide **100 across**; the dashboard needed a *production build* to say anything true (the dev server scores 65 for performance and Lighthouse's own `modern-http` insight puts 1,030 ms of that on HTTP/1.1) and scores **96/96/100/69** served behind this repo's own `nginx.conf` — where **SEO 69 is the product being correct**, because the only failing audit is `is-crawlable` on a deliberately-noindex route, so that box cannot be ticked without making the product worse. **Three runs**: met — 8m23s / 8m37s / 8m27s, all exit 0, identical numbers, no flake. **`@smoke` vs production**: 5 passed, 2 environmental, **0 real**, and the `smoke-viewer` failure was classified rather than assumed (it stalls on the *upload* inside the suite's shared IP; standalone against production it draws in 3.8 s). **p95**: local shape tightened to 6–12 ms across all six §10.2 endpoints with 0 failures; the production figure is **not** run, because there is no `PERF_EMAIL` and the owner docs list it as the owner's — §4's own fallback. Its §2 also asked for `BROWSERS=all`, which had **never actually run the other engines here** — 179 of 181 'failures' were `Executable doesn't exist`; installed and re-run it is 264 passed / 8 failed, with a real WebKit finding on the signing ceremony's keyboard path. Four queue rows added, none fixed: this prompt changes no product code and did not. See PROGRESS "2026-08-25 (later still) — Launch-gate evidence" and `docs/reviews/evidence/launch-gate-2026-08-25.md`. Historical.**
