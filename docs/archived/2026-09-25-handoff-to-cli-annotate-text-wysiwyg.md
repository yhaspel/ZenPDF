**Executed 2026-09-25/26 — see PROGRESS.md session log "2026-09-25 — Text boxes: one layout, drawn twice", PR #49 (`156d569`). The patch's sha256 matched the pre-apply copy (`c5700011…fec3a`), `git am` applied clean on `bcba629`, and `cd65b4c` shipped unchanged underneath the CLI's own commits. **Step 2 did not go green first time, and the gate was right:** the full `infra/test.sh --e2e` stopped at its skip guard — 7 skips against 6 allowed — because `test_the_frontend_serves_the_same_font_bytes` can only skip in the api container (it mounts `backend/` only). The check moved to `infra/test.sh` beside the deny-list one. The new `annotate-text-wysiwyg` spec then ran **8 / 8 on its first compose-stack run**, Chromium 1228 (the sandbox had 1194), tolerances untouched: ink-bbox Δ 0 px on 28 boxes and 1 px on 12, worst shape shift 0.993 px. **Step 3 became three passes**, each finding put to an adversarial verifier before it was fixed, each fix pinned by a test that fails on the commit it corrects: five lenses over the patch (≈ 20 confirmed — among them a high: under Select a box was re-laid against a 595 × 842 guess and saved too short on Letter/landscape pages; an inherited `/Pages` CropBox that saved boxes blank; Arabic drawn as disconnected letters; fallback glyphs clipped; the client and engine disagreeing on direction; tabs; stale `/ZenLines`; a 200-line cap that failed every mark in a save), then a second pass over those fixes (five regressions caught before merge — among them a `/Rect` conversion that lost the CropBox origin on rotated pages, and growth off the page that made the annotation list 500), then a third, focused pass (three more). Two findings were refuted (the OFL year is upstream's; document-wide subsetting is the spec's and Edit's pattern). Final gate on `43d8a71`: backend **1258 passed / 6 skipped**, coverage 91.66 % / 92.11 %, unit **658 / 68 files**, lint clean, **43 prerendered routes**, `--pg` 6, Playwright **96 passed / 1 skipped**. **Production (step 5):** all six app services deployed `156d569`; bundle `main-PMI4V7BP.js` → `main-YRIDWD57.js`; as a guest the five boxes saved with `lines` (the new engine), downloaded, and rendered by PDFKit whole — nothing cut off, the Hebrew right-aligned with "12," where the browser put it; both themes at 1280 and 390 px, console clean. Evidence `docs/reviews/evidence/annotate-text/prod-*`. **Corrections this prompt owes:** its itemised test counts (32 / 14 / 10) were 31 / 15 / 11 as delivered; "frontend serves the same bytes" is now a gate check, not a pytest; the `lines` bound is 5001 × 5000, not 200 × 2000; "moving … re-wraps against the new right edge" holds for adding breaks only — soft breaks become hard as you type, which is a Human review queue row (2026-09-25), with four more filed there (a second face for shaping scripts; PyMuPDF's recursive outline walk; and two pre-existing defects found on the way: an off-page annotation 500s the list, stamps on cropped-then-rotated pages save displaced). The delivery pair (`.zen-annotate-text.patch`, `.zen-annotate-text-HANDOFF.md`) was removed as step 6 says, after confirming the committed prompt is the pre-apply copy minus its header. Historical.**
