**Executed 2026-08-26 — see PROGRESS.md session log "2026-08-25 — Post-programme adversarial review", under its **Landed (row 10)** section. The patch applied clean onto `22ecb16` and the gate was green end to end on a restarted stack: backend **1164 passed / 6 skipped** (the six allowed query-plan skips, gotenberg healthy so the conversion tests ran), coverage apps 91.54 % / pdf_engine 91.87 %, `ng lint` clean, unit **578 / 66 files**, build **43 prerendered routes** + `verify:prerender`, pg_only **6 passed**, Playwright **86 passed / 1 skipped**. The one skip is `phase-11:164`'s `isDevServer` 404-status skip, **not** the webkit skip this prompt predicted — `infra/test.sh --e2e` runs chromium only. All three browser checks passed in both themes at 1280 and 390 with zero console errors, and the adopt guard was **reproduced rather than assumed**: the rotate's toast lands while the second document is on screen and its version chip holds its own seq while the first advances v2→v4 behind it. Both self-review lenses came out as predicted, including the skip-guard experiment (the self-test catches nothing; only the fixed failure path prints the skip list). **Two findings beyond the review's five were filed as queue rows and left unfixed, per this prompt's own rule**: the 404's `robots: noindex` outlives the 404, and "N page(s)" survives in 18 live strings everywhere except compare — the first of them the rotate toast seen above. Historical.**
