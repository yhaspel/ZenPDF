...
The annotate rail's tool grid — two columns, `--space-1` gap — is a set of mutually exclusive toggles, and it follows `.seg`'s **semantics** (a plain `<button>` per tool carrying `aria-pressed`; never `aria-selected`, never `role="tablist"`) with `.seg`'s **selected treatment** (bg `--color-ink-strong`, text `--color-bg` — the ink stamp, not accent). Idle: hairline `--color-border`, radius 4 px, `--color-ink`, text start-aligned. It is **not** a `.seg`: `.seg`'s 36 px is the dense-desktop-toolbar exemption in §6, and this rail is a full-width section on a phone (§3 workspace panes) where these buttons are the only way to choose a tool. **Minimum height 44 px**, like the popover menu and for the same reason.

**A tool armed by something other than the palette still lights up in it.** Uploading a custom stamp arms `image_stamp`, so the palette carries an `image_stamp` entry: **disabled until a stamp exists in the session**, taking §3's single disabled treatment (dashed `--color-border-strong` hairline, `--color-ink-muted`, `cursor: not-allowed`) with a `title` saying what would enable it; once a stamp is uploaded it wears that image as a **24 px icon** (`object-contain`, hairline, radius 2 px — the thumbnail treatment of §3) beside its label, and clicking it re-arms the tool. Native `disabled` is correct here — unlike a `role="menuitem"` (§3 context menu), a disabled `<button>` keeps its place in the accessibility tree. The icon is decorative (`alt=""`): the button's own words are its name. The rule generalises: **an editing mode may not enter a state its own palette cannot show or return to** — that is the no-dead-affordances invariant read from the other side.
