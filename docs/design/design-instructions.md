...
Separators are a middot with a space either side, in the flow of a single `<p>` — so the row **wraps as text, not as a grid**: at 390 px it breaks after "Verify a signed PDF" onto a second centred line carrying Contact · Guides, with no separator stranded at the end of a line and no horizontal scroll. Each link keeps a 10 px inline margin either side, which is what gives the wrapped line its own breathing room without a second rule.
