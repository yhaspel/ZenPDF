...
- **Guide article `/guides/<slug>`** *(added 2026-08-24)*: marketing header; **640 px reading column**, exactly the legal pages' typography — display headings, 16 px/1.75 body — so a guide and the privacy policy read as pages of one product. Order: H1, then a byline row, then the prose, then related tools, then the footer.
  - **Byline row**: 12.5 px `--color-ink-faint`, directly under the H1 — the author "the ZenPDF team" and the dates, in the product's date format (`toLocaleDateString` with the page's locale, day-month-year, never a raw ISO string in body copy). "Updated" is shown **only when it differs from published**, because "Published 6 August · Updated 6 August" is noise that reads as churn.
